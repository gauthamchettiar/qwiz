...
test('a per_question timer auto-submits the current answer when it runs out', async ({ page }) => {
  const quiz = buildQuiz({
    settings: { shuffle_questions: false, timer_mode: 'per_question', timer_seconds: 3 },
    questions: [
      {
        id: 'q1',
...
test('on_timeout=lock_zero scores 0 even if an option was selected before time ran out', async ({
  page
}) => {
  const quiz = buildQuiz({
    settings: {
...
test('reveal_screen_seconds auto-advances to the next question', async ({ page }) => {
  const quiz = buildQuiz({
