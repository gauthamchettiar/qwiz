...
| `min_answers`            | number                           | `pick_many`, `type_answer`                     | — (none)   | no       | Minimum selections/guesses that count as a complete answer. Only gates Submit alongside `require_answer=true`    |
...
| `require_answer`         | boolean                          | all variants                                   | `false`    | yes      | Require a complete answer before Submit becomes available. Off by default, so every question can be skipped      |
