...
| ------------------------ | -------------------------------- | ---------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------- |
| `points_correct`         | number                           | all variants                                                     | `1`        | Points for each correct option/answer/letter/placement without its own `%N%`                                     |
| `points_wrong`           | number                           | all variants                                                     | `0`        | Points deducted for each incorrect option/wrong guess without its own `%N%`                                      |
| `partial_credit`         | boolean                          | multiple_choice, typed, order, match, categorise, fill_in_blanks | `false`    | Award credit for some (not all) correct picks/guesses/placements instead of requiring an exact match             |
...
`min_answers`/`max_answers`/`partial_credit`, which are `multiple_choice`-only (see below).
...
- **Wrong variant** → parse error naming which variant(s) the key actually applies to (checked
  ...
  setting can span more than one group, e.g. `options_layout` spans `single_choice` and
...
- **`min_answers`, `max_answers`, or `partial_credit` on a `single_choice` question** —
...
- **`letter_reveal` never affects scoring**, only display. `character_input` scores per
