...
| ------------------- | -------------------------------- | ---------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------- |
| `point`             | number                           | all variants                                                     | `1`        | Points for each correct option/answer/letter/placement without its own `%N%`                                     |
| `penalty`           | number                           | all variants                                                     | `0`        | Points deducted for each incorrect option/wrong guess without its own `%N%`                                      |
| `partial_points`    | boolean                          | multiple_choice, typed, order, match, categorise, fill_in_blanks | `false`    | Award credit for some (not all) correct picks/guesses/placements instead of requiring an exact match             |
...
| `case_sensitive`    | boolean                          | typed, fill_in_blanks (blank_input=type)                         | `false`    | Require exact letter case                                                                                        |
| `numeric_tolerance` | number                           | typed, fill_in_blanks (blank_input=type)                         | — (off)    | Allowed absolute numeric difference. Mutually exclusive with `fuzzy_tolerance`                                   |
| `fuzzy_tolerance`   | number                           | typed, fill_in_blanks (blank_input=type)                         | — (off)    | Allowed typos as % of answer length (edit distance). Mutually exclusive with `numeric_tolerance`                 |
...
| `blank_input`       | `bank` \| `type`                 | fill_in_blanks only                                              | `bank`     | Pick words from a bank vs. type each blank directly                                                              |
