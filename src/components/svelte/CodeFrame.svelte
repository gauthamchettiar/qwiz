<script lang="ts">
  ...
  const line = $derived(source.split(/\r\n|\r|\n/)[error.line - 1] ?? '');
</script>

<div class="overflow-hidden rounded-lg border border-red-200 bg-red-50 text-sm">
  <div class="flex items-start gap-3 border-b border-red-200 bg-red-100/60 px-3 py-1.5 font-mono text-xs text-red-800">
